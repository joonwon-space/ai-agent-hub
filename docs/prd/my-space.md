...
### Phase 3 (확장 기능) ✓ 대부분 완료
- ✓ Phase 3.1: 이미지 업로드 (레시피 커버)
- ✓ Phase 3.2: 통합 검색 (PostgreSQL `ILIKE`)
- ✓ Phase 3.3: Space 다중 생성/관리 UI
- ✓ Phase 3.4: 접근성 + 반응형 + 다크/라이트 토글
- ✓ Phase 3.5: 레시피 read-only 뷰 + 체크리스트

### 후속 작업 — 백로그
잔여 항목은 [`docs/plan/backlog.md`](../plan/backlog.md) 단일 문서에 정리되어 있습니다.
주요 카테고리: 검색 고도화, 이미지/미디어 보강, Recipe 요리 보조, Agent 통합, 접근성 보강, AI 보조, 의존성 마이그레이션.
