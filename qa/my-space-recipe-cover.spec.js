...
// ---------------------------------------------------------------------------
// Minimal *decodable* JPEG (1×1 pixel) — the previous 22-byte SOI+APP0+EOI
// passed the magic-byte check but failed sharp/libvips decoding ("JPEG
// datastream contains no image"). This 148-byte buffer includes quantization
// tables, SOF0, Huffman tables, and a single-pixel SOS so sharp can decode it.
// ---------------------------------------------------------------------------
...
test.describe('My Space — Phase 3.1 Recipe Cover Upload', () => {
  ...
  test('fake JPEG (text content) → server 400, no preview update', async ({ page }) => {
    ...
    // This test deliberately triggers a 400 response, which the browser logs
    // as a console error ("Failed to load resource: 400 Bad Request").
    // Filter that out of the no-errors check — only assert no unexpected errors.
    errors.console = errors.console.filter((m) => !/status of 400/.test(m));
