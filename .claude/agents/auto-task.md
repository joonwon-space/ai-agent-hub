...
3. **Build verification** (MANDATORY before every commit)
   ...
   **If any `.js` files under `backend/` changed:**
   ```bash
   cd backend
   npm run lint                                   # ESLint
   npm test -- --bail 2>&1 | tail -30             # tests
   cd ..
   ```
   If lint or tests fail → fix and re-run. If it fails twice → stop and report.

   **If any `.js` files or assets under `frontend/` changed:**
   ```bash
   cd frontend
   npm run lint 2>&1 | tail -20                    # ESLint
   cd ..
   ```
   If lint fails → fix and re-run. If it fails twice → stop and report.
