...
- **JavaScript only**: no TypeScript; use JSDoc for non-obvious public APIs
- **Async**: every async function wrapped in try/catch; Express handlers pass errors via `next(err)`
- **Immutability**: new objects created, no in-place mutation
- **Error handling**: comprehensive, user-friendly messages
- **Validation**: explicit checks at request boundaries (`req.body`, `req.params`, `req.query`)
- **SQL**: parameterized queries (`$1`, `$2`) — never string concatenation
