...
- What type of error? (runtime, network, database, auth, external API)
- Is it a Node.js error (server) or browser error (client)?
...
- Search for similar errors in the codebase: `grep -rn "ErrorType\|error message" backend frontend --include='*.js'`
- Check if this error type is handled elsewhere
- Look for try-catch blocks that might be swallowing related errors

### 4. Log analysis

- Check backend logs for related errors: `grep -rn "logger\.\|console\." backend/src --include='*.js' -l`
...
```
{
  ...
  "error_type": "runtime | network | database | auth | external-api | unknown",
  "error_origin": {
    "file": "file path",
    "line": 0,
