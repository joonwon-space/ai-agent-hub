...
**외부 API 오류 (ConnectTimeout, 5xx 응답)**
- `backend/src/agents/**` — 타임아웃 설정, 재시도 로직 확인
- 외부 API 클라이언트 코드 — 토큰 만료/갱신 로직 확인
- 네트워크 문제라면 코드 수정 불필요 → 알림에 "네트워크 일시 장애" 표기

**Ollama 연결 오류 (ECONNREFUSED, fetch failed)**
- `docker compose ps` 로 Ollama 컨테이너 상태 확인
- Ollama 호스트/포트 환경 변수 확인 (`OLLAMA_BASE_URL` 등)
- 컨테이너 재시작으로 해소됐으면 코드 수정 불필요

**DB 오류 (Connection, ECONNREFUSED)**
- DB 커넥션 풀 설정 확인 (`backend/src/**`의 pg/Postgres 설정 파일)
- `docker compose ps` 로 postgres 컨테이너 상태 확인
- 컨테이너 재시작으로 해소됐으면 코드 수정 불필요

**인증 오류 (401 급증, 로그인 실패 반복)**
- `backend/src/routes/auth*` — rate limit / 토큰 검증 로직 확인

**500 Internal Server Error**
- 해당 엔드포인트 핸들러 파일 확인
- 예외 처리 누락 여부 확인 (Express에서 `next(err)` 누락 등)
...
3. 로컬 검증:
   ...
   cd backend && npm test -- --bail 2>&1 | tail -30
   ```
4. 검증 통과 후 커밋 + push:
   ```bash
   git add <수정된 파일>
