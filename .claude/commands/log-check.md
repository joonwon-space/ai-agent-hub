...
```bash
# 컨테이너 내부 로그 파일 직접 읽기
docker compose exec backend tail -n 2000 /var/log/ai-agent-hub/app.log 2>/dev/null
```
...
- 외부 API 오류: 5xx 응답, 비정상 응답 코드
- DB 연결 실패: `pg`, `connection refused`, `ECONNREFUSED`
- Ollama 연결 실패: `ECONNREFUSED`, `fetch failed`
- 인증 위조 시도: 짧은 시간 내 동일 IP의 로그인 실패 5회 이상
- 500 Internal Server Error 응답 3회 이상

### 🟡 Warning (주의 필요)
- `"level": "warning"` 또는 `WARN` 레벨 로그
- 외부 API 재시도 발생: `retrying`, `timeout`
