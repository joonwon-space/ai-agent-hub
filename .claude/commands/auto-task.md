...
3. **Build verification** (MANDATORY before every commit)
   ...
   **If any `.js` files changed under `backend/`:**
   ```bash
   cd backend && npm run lint 2>&1 | tail -20
   cd backend && npm test -- --bail 2>&1 | tail -30
   ```
   If lint or tests fail → fix and re-run. Fails twice → stop and report.

   **If any `.js` files or assets changed under `frontend/`:**
   ```bash
   cd frontend && npm run lint 2>&1 | tail -20
   ```
   If lint fails → fix and re-run. Fails twice → stop and report.
